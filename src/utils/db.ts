...
import { DBAnnoucenment, DBMatch, DBPartsRequest, TeamToMatch } from "@/types/db";
...
async function getItem(table: string, idName: string, id: string, ret: ZodTypeAny) {
...
export function prepInsertAnnouncement(item: DBAnnoucenment) {
  return prepInsert("Announcements", item);
}
...
export function prepInsertPartsRequest(item: DBPartsRequest) {
  return prepInsert("PartsRequest", item);
}
